...
Updater.updateServers = function () {
	...
	async.waterfall([
		...
		if (err) {
			if (failed < 3) {
				console.log('[Minecraft Widgets] [' + server.name + '] ' + err);
				if (++failed === 3) console.log('[Minecraft Widgets] Too many errors, squelching further logs until a success occurs.);
			}
		}else{
			if (failed === 3) console.log('[Minecraft Widgets] An recent update was successful, turning on logging of error messages.);
			failed = 0;
		}
		if (status) {
			if (status.motd) {
...
function doServerQuery(data, next) {
	...
	if (Config.settings.get().noQuery) return next(null, data);
